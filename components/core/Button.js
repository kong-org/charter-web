export default function Button({ primary, children, className, ...props }) {
  if (primary) {
    return (
      <button
        className={
          "outline-none px-8 py-3 bg-kong-green hover:bg-kong-green-800 text-black uppercase text-sm md:text-base font-mono font-bold transition duration-250 ease-in-out " +
          className
        }
        {...props}
      >
