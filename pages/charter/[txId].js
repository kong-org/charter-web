    ...
    <div className="flex w-full">
      <div
        className={
          ...
          " transition duration-500 flex-0 w-full space-x-2 lg:space-x-4 flex justify-end"
        }
      >
        <Button
          text="Sign"
...
function Body({ txId, data, status }) {
  if (status === 200) {
    ...
    return (
      <>
        ...
        <div className="md:mx-4 mt-16 md:mt-20 font-body text-gray-primary text-left max-w-2xl">
          {title && (
            <h2 className="text-4xl my-16 font-bold text-center">{title}</h2>
          )}
          <ReactMarkdown className="prose prose-xl lg:prose-2xl markdown-rte text-white">
            {body}
          </ReactMarkdown>
          <p className="font-bold text-left text-white uppercase font-title text-2xl mt-8">
            {timestamp}
          </p>
          <p className="font-mono text-white text-opacity-50 text-base mt-8">
            This document lives on Arweave at transaction{" "}
            <a
              className="underline"
            ...
            <div className="mt-20 max-w-3xl">
              <ul className="flex flex-wrap font-mono">
                {parsedAuthors.map((author) => (
                  <li
                    className="my-1 mx-2 overflow-hidden py-2 px-4  text-gray-120 hover:text-gray-20 bg-gray-200"
                    key={author.name}
                  >
    ...
    return (
      <div className="w-1/4 font-title">
        <h3 className="text-2xl font-bold">Not Found</h3>
        <p className="text-lg leading-tight my-4">
          Either this transaction doesn't exist or the data format is incorrect.
        </p>
      </div>
    );
  }
...
export default function Charter() {
  ...
  return (
    <>
      <div className="flex flex-col items-center bg-kong-black justify-center pt-8 pb-24">
        <HeadComponent />
        <main className="flex flex-col items-center min-h-screen w-full flex-1 px-4 lg:px-8 text-center">
          ...
          <div className="w-3/5">
            <h1 className="uppercase text-white text-4xl font-title mt-16 mb-16 md:mb-20 md:text-7xl font-semibold text-gray-primary">
              Founding Charter
            </h1>
          </div>
          {maybeCharter.loading ? (
            <BarLoader
              speedMultiplier=".75"
              height="2px"
              width="300px"
            ...
            <Body txId={txId} {...maybeCharter.result} />
          )}
        </main>
      </div>
    </>
